...
class TestNoSkillsOptOut:
    """Compatibility tests for the deprecated `--no-skills` marker."""

    def test_no_skills_writes_legacy_marker_but_profile_stays_empty(self, profile_env):
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)

        marker = profile_dir / NO_BUNDLED_SKILLS_MARKER
        assert marker.is_file(), "expected legacy .no-bundled-skills marker in profile root"
    ...
    def test_seed_profile_skills_is_noop_for_opted_out_profile(self, profile_env, monkeypatch):
        """Profiles use shared skill roots; direct seeding must never copy bundled skills."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
        monkeypatch.setattr("subprocess.run", lambda *a, **kw: pytest.fail("subprocess.run called"))

        result = seed_profile_skills(profile_dir, quiet=True)

        assert result is not None
        assert result.get("skipped_shared_baseline") is True
        assert result.get("skipped_opt_out") is True
        assert result.get("copied") == []
        assert list((profile_dir / "skills").iterdir()) == []

    def test_seed_profile_skills_is_noop_for_regular_profile(self, profile_env, monkeypatch):
        """A regular profile also gets no bundled-copy seed; shared skills are external."""
        profile_dir = create_profile("coder", no_alias=True)
        ...
        assert result is not None
        assert result.get("skipped_shared_baseline") is True
        assert result.get("skipped_opt_out") is False
        assert result.get("copied") == []
        assert list((profile_dir / "skills").iterdir()) == []

    def test_delete_marker_does_not_reenable_profile_seeding(self, profile_env, monkeypatch):
        """The legacy marker no longer controls copying because copying is gone."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
        assert has_bundled_skills_opt_out(profile_dir) is True
