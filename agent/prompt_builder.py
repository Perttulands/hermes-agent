...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills\n"
            "Load a visible skill with `skill_view(name)` only when the user's request "
            "directly and clearly matches its description. Do not load skills for "
            "tangential overlap. Some installed user-only skills are intentionally absent "
            "from this list; they remain available when the user invokes them explicitly "
            "by name or slash command.\n"
            "For Hermes Agent configuration or troubleshooting, load `hermes-agent` first. "
            "Maintain skill content with `skill_manage` when requested or when a loaded "
            "skill has a verified defect.\n"
            "\n"
            "<available_skills>\n"
            + "\n".join(index_lines) + "\n"
            "</available_skills>"
